...
public class TodoService {
    ...
    public List<TodoResponse> getTodos(int page, int size, getTodosRequestDto requestDto) {
        ...
        todosCheck(requestDto);
        return todoQueryRepository.findAll(
                requestDto.getStartDate()
                ,requestDto.getEndDate()
                , requestDto.getWeather(), pageable);
    }
    public void todosCheck(getTodosRequestDto requestDto) {
        /*
        기간의 시작과 끝이 둘다 있을때
        기간시작일이 끝보다 느릴수 없고
    ...
    public List<searchTodosResponse> searchTodos(int page, int size, searchTodosRequestDto requestDto) {
        Pageable pageable = PageRequest.of(page - 1, size);

        return todoQueryRepository.findSearchTodo(requestDto.getTitle(),requestDto.getCreatedAt(), requestDto.getNickname(),pageable);
    }
